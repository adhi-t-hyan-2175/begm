export const getSelectionMultiplier = (selection) => {
  ...
  // Wheelocity (5x, 3x, 2x)
  if (sel.includes('5x') || sel.includes('five') || sel.includes('5 hits')) return 4.9;
  if (sel.includes('3x') || sel.includes('three') || sel.includes('3 hits')) return 2.9;
  if (sel.includes('2x') || sel.includes('two') || sel.includes('2 hits')) return 1.9;
  
  // Tie/Andar Bahar
  if (sel.includes('tie')) return 4.5;
  ...
  // Size (Dice)
  if (sel.includes('small') || sel.includes('big') || sel.includes('large')) return 1.9;
  
  // Parity Numbers
  if (sel.includes('even') || sel.includes('odd')) return 1.9;
  if (sel.match(/^\d+$/)) return 9; // Number bets
...
export const calculatePayout = (selection, contractAmount, feePercent = 2, customMultiplier = null) => {
  ...
  // Math exactly matches user requirements
  const winningAmount = Math.max(0, Math.round(contractAmount * multiplier));
