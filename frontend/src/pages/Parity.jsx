...
const Parity = () => {
  ...
  const { timeLeft, isBettingOpen, period, previousPeriod, formatTime, secondsIntoPeriod } = useGameTimer(120, 60);
  ...
  useEffect(() => {
    ...
    if (myPrevBets.length > 0) {
      ...
      const won = String(bet.selection).toLowerCase() === String(resultLabel).toLowerCase();
      const multiplier = MULTIPLIERS[bet.selection] || 2;
      setTimeout(() => setResultCard({
        won, period: previousPeriod, game: GAME,
        selection: bet.selection, selectionColor: getSelColor(bet.selection),
        resultLabel, resultColor: getSelColor(resultLabel),
    ...
    <div className="rui-screen">
      ...
      {myActiveBets.length > 0 && (
        <div className="rui-active-bets">
          ...
          {myActiveBets.map(o => (
            <div key={o.id} className="rui-active-bet-row">
              <span style={{ background: getSelColor(o.selection), color: '#fff', borderRadius: 6, padding: '2px 8px', fontSize: '0.8rem', fontWeight: 700 }}>{o.selection}</span>
              ...
              {myGameOrders.length === 0
                ...
                : myGameOrders.map(ord => (
                    <tr key={ord.id}>
                      ...
                      <td>₹{ord.amount}</td>
                      <td>
                        {ord.status === 'Pending' ? <span style={{ color: '#f59e0b', fontWeight: 700, fontSize: '0.82rem' }}>Pending</span>
                          : ord.status === 'Won' ? <span style={{ color: '#16a34a', fontWeight: 800, fontSize: '0.88rem' }}>▲ +₹{ord.winAmount?.toFixed(2)}</span>
                          : <span style={{ color: '#dc2626', fontWeight: 700, fontSize: '0.88rem' }}>✗ Lost</span>}
                      </td>
                    </tr>
                  ))}
            </tbody>
        ...
        <div className="rui-modal-backdrop" onClick={() => setShowHistoryModal(false)}>
          <div className="rui-modal" onClick={e => e.stopPropagation()}>
            <div className="rui-modal-header">
              <span className="rui-modal-title">Parity History</span>
